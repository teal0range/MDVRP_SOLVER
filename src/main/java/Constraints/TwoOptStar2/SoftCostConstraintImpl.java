...
public class SoftCostConstraintImpl extends SoftCostConstraint {
    ...
    public double fulfilled(OperationContext context) {
        ...
        Node endNode1 = context.sideRoute.getNode(context.operatePos[1]);
        double distanceBefore = context.problem.getDistance(startNode1, context.mainRoute.getNode(context.operatePos[0] + 1)) +
                context.problem.getDistance(endNode1, context.sideRoute.getNode(context.operatePos[1] + 1));
        double distanceAfter = context.problem.getDistance(startNode1, context.sideRoute.getNode(context.operatePos[1] + 1)) +
                context.problem.getDistance(context.mainRoute.getNode(context.operatePos[0] + 1), endNode1);
